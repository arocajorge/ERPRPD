...
SELECT dbo.fa_notaCreDeb.IdEmpresa, dbo.fa_notaCreDeb.IdSucursal, dbo.fa_notaCreDeb.IdBodega, dbo.fa_notaCreDeb.IdNota, dbo.fa_notaCreDeb.CodDocumentoTipo, 
                  dbo.fa_notaCreDeb.Serie1 + '-' + dbo.fa_notaCreDeb.Serie2 + '-' + dbo.fa_notaCreDeb.NumNota_Impresa AS vt_NumFactura, LTRIM(RTRIM(dbo.tb_persona.pe_nombreCompleto)) AS pe_nombreCompleto, dbo.fa_notaCreDeb.IdCliente, 
                  dbo.fa_notaCreDeb.no_fecha, web.vwcxc_cobro_det_valor_retenciones.ValorRteFTE, web.vwcxc_cobro_det_valor_retenciones.ValorRteIVA, web.vwcxc_cobro_det_valor_retenciones.PorcentajeRetFTE, 
                  web.vwcxc_cobro_det_valor_retenciones.PorcentajeRetIVA, web.vwcxc_cobro_det_valor_retenciones.TotalRTE, web.vwcxc_cobro_det_valor_retenciones.cr_fecha
FROM     dbo.fa_cliente INNER JOIN
                  ...
                  web.vwcxc_cobro_det_valor_retenciones ON dbo.fa_notaCreDeb.IdEmpresa = web.vwcxc_cobro_det_valor_retenciones.IdEmpresa AND dbo.fa_notaCreDeb.IdSucursal = web.vwcxc_cobro_det_valor_retenciones.IdSucursal AND 
                  dbo.fa_notaCreDeb.IdBodega = web.vwcxc_cobro_det_valor_retenciones.IdBodega_Cbte AND dbo.fa_notaCreDeb.IdNota = web.vwcxc_cobro_det_valor_retenciones.IdCbte_vta_nota AND 
                  dbo.fa_notaCreDeb.CodDocumentoTipo = web.vwcxc_cobro_det_valor_retenciones.dc_TipoDocumento
GO
